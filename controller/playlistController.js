...
export const recommendMusic = async (req, res) => {
    ...
    try {
        ...
        const recommendedPlaylist = playlistResponse.body.playlists.items[0]; // Pick the first playlist
        const playlistData = {
            user_id: userId,
            mood: mood,
            playlist_name: recommendedPlaylist.name,
            playlist_url: recommendedPlaylist.external_urls.spotify,
            playlist_image: recommendedPlaylist.images.length > 0 ? recommendedPlaylist.images[0].url : null,
        };
        ...
        res.status(500).json({ message: "Error fetching playlist recommendations" });
    }
};
